...
namespace PPAsta.Service.Services.PP.Payment
{
    ...
    public class SrvPaymentService : ISrvPaymentService
    {
        ...
        {
            MdlPayment payment = await _paymentRepository.GetPaymentGameAsyncByBuyerId(paymentGame.BuyerId.Value);
            ...
            if (payment == null)
            {
                ...
                payment = await _paymentRepository.GetPaymentGameAsyncByBuyerId(paymentGame.BuyerId.Value);
                id = payment.Id;
            }
            else
            {
                payment.TotalPurchasePrice += paymentGame.PurchasePrice.Value;
                payment.TotalShareOwner += paymentGame.ShareOwner.Value;
                payment.TotalSharePP += paymentGame.SharePP.Value;
                payment.RUD = DateTime.Now;
